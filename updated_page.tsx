...
function getRetailValue(item: Option) {
  return item.monthly;
}
...
function getProviderMix(combo: Combo) {
  const providers = combo.chosen.map((item) => getItemProviderKey(item) ?? "direct");
  return [...new Set(providers)];
}

function getProviderLabels(combo: Combo) {
  const labels = combo.chosen.map(
    (item) => getIncludedWithLabel(item) ?? "via Direct"
  );
...
function getBestForLabel(combo: Combo, selected: string[]) {
  ...
  if (extraCoveredServices > 0) {
    return "Covers what you picked, plus extra included value";
  }
    ...
    <>
      ...
      {extraCoveredServices > 0 && (
        <span className="rounded-full bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-700">
          Includes extra service you didn’t select
        </span>
      )}
    </>
  );
...
function getRecommendationScore(
  ...
  const confidence = getComboConfidence(combo);
  const providerCount = getProviderMix(combo).length;
  const planCount = combo.chosen.length;
  const extraCoveredServices = getExtraCoveredServiceCount(combo, selected);

  const promoItems = combo.chosen.filter((item) => item.category === "promo");

  const hasWeakPromo = promoItems.some(
    (item) => !item.introLengthMonths || item.introLengthMonths < 3
  );

  const hasStrongPromo = promoItems.some(
    (item) => Boolean(item.introLengthMonths && item.introLengthMonths >= 3)
  );

  const hasMembershipPath = combo.chosen.some((item) => isPaidMembershipPath(item));
  const hasCarrier = combo.chosen.some((item) => item.category === "carrier");
  const hasBundle = combo.chosen.some((item) => item.category === "bundle");

  let score = 0;

  score += primaryTotal * 8;
  score += planCount * 12;
  score += providerCount * 10;
  score += extraCoveredServices * 18;
  ...
  if (hasWeakPromo) {
    score += rankingMode === "starting" ? 85 : 120;
  }

  if (hasStrongPromo) {
    score += rankingMode === "starting" ? 20 : 35;
  }

  if (hasMembershipPath) {
    score += 12;
  }
  ...
  if (confidence === "high" && planCount <= 2 && providerCount <= 2) {
    score -= 35;
  }

  if (confidence === "high" && !hasWeakPromo && !hasStrongPromo) {
    score -= 30;
  }
...
function getRecommendedCombo(
  ...
  const noExtraOptions = combos.filter(
    (combo) => getExtraCoveredServiceCount(combo, selected) === 0
  );

  if (!noExtraOptions.length) {
    return [...combos].sort((a, b) => {
      return (
        getRecommendationScore(a, rankingMode, selected) -
          getRecommendationScore(b, rankingMode, selected) ||
        getPrimaryTotal(a, rankingMode) - getPrimaryTotal(b, rankingMode)
      );
    })[0];
  }

  const bestNoExtraByScore = [...noExtraOptions].sort((a, b) => {
    return (
      getRecommendationScore(a, rankingMode, selected) -
        getRecommendationScore(b, rankingMode, selected) ||
      getPrimaryTotal(a, rankingMode) - getPrimaryTotal(b, rankingMode)
  ...
  const stableNoExtraOptions = noExtraOptions.filter((combo) => {
    const confidence = getComboConfidence(combo);
    const hasPromo = combo.chosen.some((item) => item.category === "promo");
    return confidence === "high" && !hasPromo;
  });

  const bestStableNoExtra = [...stableNoExtraOptions].sort((a, b) => {
    return (
      getPrimaryTotal(a, rankingMode) - getPrimaryTotal(b, rankingMode) ||
      a.chosen.length - b.chosen.length
    );
  ...
  if (!bestStableNoExtra) {
    return bestNoExtraByScore;
  }

  if (bestNoExtraByScore.id === bestStableNoExtra.id) {
    return bestNoExtraByScore;
  }

  const promoItems = bestNoExtraByScore.chosen.filter(
    (item) => item.category === "promo"
  );
  ...
  const candidateTotal = getPrimaryTotal(bestNoExtraByScore, rankingMode);
  const stableTotal = getPrimaryTotal(bestStableNoExtra, rankingMode);
  const savingsVsStable = stableTotal - candidateTotal;

  if (hasWeakPromo) {
    return savingsVsStable >= 20 ? bestNoExtraByScore : bestStableNoExtra;
  }

  if (hasStrongPromo) {
    return savingsVsStable >= 8 ? bestNoExtraByScore : bestStableNoExtra;
  }
...
function getRecommendedReason(
  ...
  if (extraCoveredServices > 0) {
    return "Not recommended because it adds services you did not select, even if that extra value might be useful.";
  }

  if (
    cheapest &&
    cheapest.id !== combo.id &&
    getExtraCoveredServiceCount(cheapest, selected) > 0
  ) {
    return "Recommended because it avoids adding unselected services while still keeping the cost low.";
  }

  if (confidence === "high" && combo.chosen.length <= 2 && providerCount <= 2) {
    return "Best overall balance of cost, reliability, and simplicity.";
  }

  if (confidence === "high" && !hasPromo) {
    return "More reliable long-term pricing with fewer surprises.";
  }

  if (hasPromo) {
    return "Strong savings without adding services you didn’t ask for.";
  }

  return "Best mix of savings, simplicity, and trust.";
}
...
function renderBestSummary(
  ...
  const cheapestDelta =
    cheapest && cheapest.id !== combo.id
      ? getPrimaryTotal(combo, rankingMode) - getPrimaryTotal(cheapest, rankingMode)
      : 0;
    ...
    <div className="mt-4 rounded-2xl bg-slate-900 p-5 text-white">
      ...
      <div className="mt-2 text-slate-200">
        ...
        <span className="font-semibold">
          {formatMoney(Number.isFinite(primarySavings) ? primarySavings : 0)}
        </span>
      </div>

      {cheapest && cheapest.id !== combo.id && (
        <div className="mt-2 text-sm text-slate-300">
          {formatMoney(Math.abs(cheapestDelta))} more than the cheapest path
        </div>
      )}
...
function renderCheapestCard(
  ...
  return (
    <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="text-sm font-semibold text-slate-900">Cheapest option (not always best)</div>
        {renderStrategyBadge(getComboStrategy(combo))}
        {renderConfidenceBadge(confidence)}
        {extraCoveredServices > 0 && (
          <span className="rounded-full bg-rose-100 px-2 py-0.5 text-xs font-semibold text-rose-700">
      ...
      <div className="mt-1 text-sm font-medium text-slate-700">
        {formatMoney(
          getPrimaryTotal(recommended, rankingMode) - getPrimaryTotal(combo, rankingMode)
        )} cheaper than the recommended path
      </div>
...
function renderTradeoffLine(
  ...
  if (extraCoveredServices > 0) {
    parts.push(
      `includes ${extraCoveredServices} extra service${
        extraCoveredServices === 1 ? "" : "s"
      }`
    );
  }
  ...
  return (
    ...
    >
      ...
      {cheapest && combo.id !== cheapest.id && (
        <div className="mt-1 text-sm font-medium text-slate-700">
          {renderTradeoffLine(combo, cheapest, rankingMode, selected)}
        </div>
      )}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {getProviderLabels(combo).map((label) => (
  <span
    key={label}
...
{getExtraCoveredServiceCount(combo, selected) > 0 && (
  <span className="text-xs text-slate-500">
    {" "}• includes {getExtraCoveredServiceCount(combo, selected)} extra service{getExtraCoveredServiceCount(combo, selected) === 1 ? "" : "s"}
  </span>
)}
      </div>
    </div>
...
export default function Page() {
  ...
  const serviceCards = React.useMemo(() => {
    return Array.from(
      new Map(services.map((service) => [service.group, service])).values()
    );
  }, []);
  ...
  const best = recommended ?? displayCombos[0];

  const runnerUp = React.useMemo(() => {
    if (!displayCombos.length) return undefined;
    if (!best) return displayCombos[1];
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <h2 className="text-2xl font-semibold">2. Recommended plan</h2>
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div className="text-sm text-slate-500">
              Compare the options your dataset found by starting cost, ongoing cost, and savings.
            </div>
          </div>
          ...
          <div className="mt-5 overflow-x-auto">
            <div className="min-w-[980px] overflow-hidden rounded-2xl border border-slate-200">
              <div className="grid grid-cols-[1.4fr_1fr_0.7fr_0.8fr_0.7fr_0.7fr] bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700">
                <div>Combination</div>
                <div>Coverage</div>
                <div>Starting</div>
                <div>Ongoing</div>
                <div>Immediate savings</div>
                <div>Ongoing savings</div>
              </div>
              ...
              {displayCombos.length ? (
                displayCombos.map((combo, index) => (
                  <div
                    key={combo.id}
                    className={`grid grid-cols-[1.4fr_1fr_0.7fr_0.8fr_0.7fr_0.7fr] gap-3 border-t px-4 py-4 text-sm ${
                      index === 0
                  ...
                  >
                    <div>
                      <div className="flex flex-wrap items-start gap-2">
                        {renderComboBadges(combo, index, selected, best?.id)}
