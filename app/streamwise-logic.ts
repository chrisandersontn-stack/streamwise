...
function getCoveredSelectedRetailTotal(
  ...
  coveredSelected.forEach((service) => {
    const match = serviceCatalog.find((item) => item.group === service);
    total += toFiniteNumber(match?.monthly);
  });
...
function tryAddResult(
  ...
  const combo = buildComboResult(chosen, selectedSet, serviceCatalog);
...
function searchCombos(
  ...
  function dfs(
    ...
    if (coversSelectedServices(covered, selectedSet)) {
      tryAddResult(chosen, selectedSet, results, seen, serviceCatalog);
